...
## 2026-03-10 - Add Security Headers Filter
**Vulnerability:** The application was missing basic security headers like `X-Content-Type-Options`, `X-Frame-Options`, `X-XSS-Protection`, `Strict-Transport-Security`, and `Content-Security-Policy`.
**Learning:** Adding security headers is a simple but effective defense-in-depth measure. Without them, the application is more susceptible to common web vulnerabilities like Cross-Site Scripting (XSS), clickjacking, and MIME-sniffing attacks. A Servlet Filter is an easy and global way to apply these headers to all incoming HTTP requests in a Spring Boot application.
**Prevention:** Implement a global `Filter` annotated with `@Component` to automatically add standard security headers to all HTTP responses.
