...
## 2026-03-12 - Missing Security Headers in REST API
**Vulnerability:** The application was missing critical HTTP security headers (X-Content-Type-Options, X-Frame-Options, X-XSS-Protection, Content-Security-Policy, Strict-Transport-Security). This exposes the API and potential clients to various attacks like clickjacking, cross-site scripting (XSS), and MIME-type sniffing.
**Learning:** Spring Boot does not enable these headers by default unless Spring Security is explicitly included and configured. In microservices or lightweight APIs where full Spring Security is omitted, these headers must still be implemented to maintain defense in depth.
**Prevention:** Always implement a `Filter` (e.g., `SecurityHeadersFilter`) to inject baseline security headers into all HTTP responses when a comprehensive security framework like Spring Security is not in use. Ensure tests verify the presence of these headers.
